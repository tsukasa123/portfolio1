...
<div class="card rounded-0" style="width: 100%;">
    ...
    {{-- @if() --}}
    <img class="card-img-top" src="{{ asset('storage/' . $profile->background_image) }}" alt="" width="100%" height="200px" style="background-color:gray; position: relative;">
    <img src="{{ asset('storage/' . $profile->avatar) }}" alt="" width="140px" height="140px" class="rounded-circle" style="background-color:gray; position:absolute; top:120px; left:20px; border: 3px solid white;">
    ...
    <div class="card-body mt-2">
        <div class="d-flex justify-content-end">
            <a href="{{ route('user.profile') }}" class="btn btn-outline-primary btn-md rounded-pill">Settings</a>
        </div>
        
        <h4 class="card-title font-weight-bold m-0">{{ Auth::user()->name }} <br>
            <small class="text-muted">@account_name</small></h4>
        <p class="m-0 py-1">{{ $profile->introduction }}</p>
...
<div class="card rounded-0">
    <div class="card-body d-flex p-1">
        <div class="col-md-1 p-1">
            <img src="{{ asset('storage/' . $profile->avatar) }}" alt="IMG" class="rounded-circle" width="50px" height="50px">
        </div>
        <div class="col-md-11">
            <div class="d-flex justfy-content-between">
                <h5 class="pr-2">{{ Auth::user()->name }}</h5>
